...
; multi-function feature key for selecting a software defined function shown on the bottom left
; of the display.
Global Const $KEYCODE_SOFT_LEFT = 1
; Key code constant: Soft Right key. Usually situated below the display on phones and used as a
; multi-function feature key for selecting a software defined function shown on the bottom right
; of the display.
...
Global Const $KEYCODE_FOCUS = 80 ;Camera; focus
; Key code constant: '+' key.
...
Global Const $KEYCODE_PICTSYMBOLS = 94; switch symbol-sets (Emoji, Kao - moji)
; Key code constant: Switch Charset modifier key. Used to switch character sets (Kanji,
; Katakana).
Global Const $KEYCODE_SWITCH_CHARSET = 95;  switch char-sets (Kanji, Katakana)
...
; Key code constant: B Button key. On a game controller, the B button should be either the button
; labeled B or the second button on the upper row of controller buttons.
Global Const $KEYCODE_BUTTON_B = 97
; Key code constant: C Button key. On a game controller, the C button should be either the button
; labeled C or the third button on the upper row of controller buttons.
Global Const $KEYCODE_BUTTON_C = 98
; Key code constant: X Button key. On a game controller, the X button should be either the button
; labeled X or the first button on the lower row of controller buttons.
Global Const $KEYCODE_BUTTON_X = 99
; Key code constant: Y Button key. On a game controller, the Y button should be either the button
; labeled Y or the second button on the lower row of controller buttons.
Global Const $KEYCODE_BUTTON_Y = 100
; Key code constant: Z Button key. On a game controller, the Z button should be either the button
; labeled Z or the third button on the lower row of controller buttons.
Global Const $KEYCODE_BUTTON_Z = 101
; Key code constant: L1 Button key. On a game controller, the L1 button should be either the
; button labeled L1 (or L) or the top left trigger button.
Global Const $KEYCODE_BUTTON_L1 = 102
; Key code constant: R1 Button key. On a game controller, the R1 button should be either the
; button labeled R1 (or R) or the top right trigger button.
Global Const $KEYCODE_BUTTON_R1 = 103
; Key code constant: L2 Button key. On a game controller, the L2 button should be either the
; button labeled L2 or the bottom left trigger button.
Global Const $KEYCODE_BUTTON_L2 = 104
; Key code constant: R2 Button key. On a game controller, the R2 button should be either the
; button labeled R2 or the bottom right trigger button.
Global Const $KEYCODE_BUTTON_R2 = 105
; Key code constant: Left Thumb Button key. On a game controller, the left thumb button indicates
; that the left (or only) joystick is pressed.
Global Const $KEYCODE_BUTTON_THUMBL = 106
; Key code constant: Right Thumb Button key. On a game controller, the right thumb button
; indicates that the right joystick is pressed.
...
; start of a line or to the top of a list.
Global Const $KEYCODE_MOVE_HOME = 122
; Key code constant: End Movement key. Used for scrolling or moving the cursor around to the end
; of a line or to the bottom of a list.
...
; should normally be implemented as a toggle such that the first press mutes the speaker and the
; second press restores the original volume.
Global Const $KEYCODE_VOLUME_MUTE = 164
; Key code constant: Info key. Common on TV remotes to show additional information related to
...
; Key code constant: Window key. On TV remotes, toggles picture-in-picture mode or other
; windowing functions.
...
; Key code constant: Toggle captions key. Switches the mode for closed-captioning text, for
; example during television shows.
...
; Key code constant: Set-top-box power key. On TV remotes, toggles the power on an external
; Set-top-box.
Global Const $KEYCODE_STB_POWER = 179
; Key code constant: Set-top-box input key. On TV remotes, switches the input mode on an external
; Set-top-box.
Global Const $KEYCODE_STB_INPUT = 180
; Key code constant: A/V Receiver power key. On TV remotes, toggles the power on an external A/V
; Receiver.
...
; Key code constant: Red "programmable" key. On TV remotes, acts as a contextual/programmable
; key.
Global Const $KEYCODE_PROG_RED = 183
; Key code constant: Green "programmable" key. On TV remotes, actsas a contextual/programmable
; key.
Global Const $KEYCODE_PROG_GREEN = 184
; Key code constant: Yellow "programmable" key. On TV remotes, acts as a contextual/programmable
; key.
Global Const $KEYCODE_PROG_YELLOW = 185
; Key code constant: Blue "programmable" key. On TV remotes, acts as a contextual/programmable
; key.
...
; between English and Japanese on a QWERTY keyboard. On some devices, the same function may be
; performed by pressing Shift+Spacebar.
...
; device behave more politely in certain settings such as on a crowded train. On some devices,
; the key may only operate when long-pressed.
